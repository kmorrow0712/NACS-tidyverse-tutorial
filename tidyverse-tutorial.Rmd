...
Today we'll import a .CSV file using `read.delim()`^1^ to a variable called **df.**

~1 You can use `read.table()` to import other datasets, but read.delim() makes assumptions about the file (e.g., it's common delimited...) that makes it a quicker import. Text files should be imported with read.table().~
...
`dplyr` uses a variety of verbs and pipes (`%>%`) to transform your data.
...
For instance, there is obviously a relationship between Defense and Attack stats, but we're also intermixing 18 groups! We've also seen that simply adding colors doesn't help, just too many groups!
